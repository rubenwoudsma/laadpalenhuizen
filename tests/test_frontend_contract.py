...
class FrontendContractTest(unittest.TestCase):
    def test_v21_frontend_ranks_complete_indicative_and_keeps_partial_as_floor(self):
        app = (ROOT / "app.js").read_text(encoding="utf-8")
        self.assertIn("quality.decision_grade === 'exclude'", app)
        self.assertIn("quality.cost_completeness === 'partial'", app)
        self.assertIn("function knownCostFloor", app)
        self.assertIn("vanaf ${euro(knownCost.min)}", app)
        self.assertIn("indicatief vergelijkbaar", app)
        self.assertIn("OCPI-voorwaarden verwerkt als bandbreedte", app)
        self.assertIn("mid: (min + max) / 2", app)
        self.assertIn("prijsbanden op middenwaarde", app)
